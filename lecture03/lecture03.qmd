...
- Then, for any $y \in K$, we have that $F(-x) > 0$ and $F(-x) > 0$;
...
$$
\mathbb{E}[m(R - R_f)] = 0 \implies \mathbb{E}[R - R_f] = -\text{Cov}(m, R)
$$
...
- The return process with the highest (absolute) Sharpe Ratio is such that $\rho(m, R - R_f) = \pm 1 \implies R - R_f = a + b \cdot m + \text{noise}$;
...
- Consider $N$ assets with random payoffs $R = (R_1, \ldots, R_N)^\top$ and a risk-free rate $R_f$;
...
$$
m(a,b) \equiv a + b^\top \left(R - R_f \mathbf{1}\right)
$$
...
\end{equation*}
```
where $\mu \equiv \mathbb{E}[R - \mathbb{E}[R]]$ and $\Sigma \equiv \mathbb{E}\left[(R - \mathbb{E}[R])(R - \mathbb{E}[R])^\top\right]$.
...
```{=latex}
\begin{equation*}
m^* = \frac{1}{R_f} - \frac{1}{R_f} \cdot \mu^\top \Sigma^{-1} \left(R - R_f \mathbf{1}\right)
\end{equation*}
```
