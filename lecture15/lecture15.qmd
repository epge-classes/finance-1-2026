...
Suppose $x_1=x_2=x$ and let $c=\beta_1+\beta_2$. Then
...
=
\frac{c^2+(\beta_1-\beta_2)^2}{2}
\geq
\frac{c^2}{2}
$$

- Lasso: equality for any split with $\beta_1\beta_2 \geq 0$
- Ridge: equality if, and only if, $\beta_1=\beta_2=c/2$
...
- This is the Belloni-Chernozhukov-Hansen move [@belloni_high-dimensional_2014; @belloni_inference_2014; @chernozhukov_doubledebiased_2018]
...
- If $\theta$ is penalized, shrinkage can push the treatment effect toward zero
- Even if $\theta$ is left unpenalized, selection mistakes can still bias $\hat\theta$
- Good prediction for $y_i$ is not the same thing as good inference on $\theta_0$

## Approach 2: Post-Single-Selection
...
2. Keep only the selected controls $x_{i,\hat S_y}$
3. Run OLS of $y_i$ on $d_i$ and $x_{i,\hat S_y}$

. . .

- This removes the direct penalty bias on $\theta_0$
- But it still uses only the controls selected from the outcome equation
- This estimator has very bad properties for inference [@leeb_can_2008; @leeb_guest_2008]

## Why Single Selection Still Fails
...
- A variable can have a small $\beta_j$ but a large $\gamma_j$
- Omitting that variable still creates bias in the treatment regression
...
$$
\sqrt{n}\left(\hat\theta_{PDS} - \theta_0\right) \xrightarrow{d} \mathcal N(0,\sigma_\theta^2)
$$

- Inference is uniformly valid over a large class of data-generating processes [@belloni_inference_2014]
- Confidence intervals are built in the usual way once $\sigma_\theta^2$ is estimated
- See the paper for details on how to estimate the variance!
...
- What if linearity is not a good assumption?
- What if you want to use more flexible ML methods, like random forests or neural nets?
- Then: @chernozhukov_doubledebiased_2018
- Secrete sauces: Neyman orthogonality and cross-fitting
- The theory gets one order of magnitude more sophisticated, but the intuition remains
...
- Regularized linear models are useful for prediction when $p$ is large
- They still leverage linearity, but they target prediction, not inference
- We can still perform valid inference relatively easily
- Many techniques already implemented by [DoubleML](https://docs.doubleml.org/stable/index.html)
- Heterogenous effects: [EconML by ALICE Labs](https://github.com/py-why/econml)
