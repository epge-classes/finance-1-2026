...
- Divide into $K$ folds $\mathcal I_1, \ldots, \mathcal I_K$
...
- But nuisance objects like propensity scores or first stages are prediction problems
...
- A small subset gives a good approximation
...
## Omitted Variable Bias, Written Down

Suppose we control only for $\widetilde X$, and let $w$ denote the omitted component of $X\beta_0$.
If $d_\perp$ and $w_\perp$ are residuals after projecting on $\widetilde X$, then
...
\theta_0
+
\frac{\operatorname{Cov}(d_\perp, w_\perp)}{\operatorname{Var}(d_\perp)}
$$

- Bias survives whenever omitted controls both affect $y$ and line up with $d$
- Selecting only from the outcome equation is not enough
...
<!-- TODO: insert Single_vs_Double_Selection figure -->

```{=latex}
\vspace{0.9cm}
\begin{center}
\fbox{\parbox{0.78\textwidth}{\centering
Placeholder for the single-selection versus double-selection Monte Carlo figure}}
\end{center}
```

- Future figure: single selection can generate substantial bias
- Post-double-selection keeps coverage much closer to the target
...
- In @KozakNagelSantosh2020, the problem is not "find one magic factor"
- It is closer to
$$
\mathbb E_t[r_{i,t+1}] \approx z_{i,t}^\top b
$$
with many weak, correlated characteristics
- Shrinkage is the point, not a nuisance

## Taming the Factor Zoo

- In @FengGiglioXiu2020, candidate factors must be studied jointly
- High-dimensional structure means one-factor-at-a-time logic is too weak
- Regularization and joint testing are natural responses once the predictor space is large

. . .

- Lecture 14 said: the zoo is real
- Lecture 15 says: high dimension needs disciplined statistics
